...
unsigned int NearestNeighbors::getKNearestNeighbors(const glm::vec3 &P, unsigned int K, vector<size_t> &neighbors, vector<float> &dists_squared) const
{
...
unsigned int NearestNeighbors::getNeighborsInRadius(const glm::vec3 &P, float radius, vector< pair<size_t, float> > &neighbors) const
{
	if(nnPoints.points == NULL)
		return 0;
	
	neighbors.clear();
	size_t nMatches = kd_tree->radiusSearch(&P[0], radius * radius, neighbors, nanoflann::SearchParams());
